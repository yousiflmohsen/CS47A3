...
class db_table:
    ...
    # def insert(self, item):
    #     # build columns & values queries
    #     columns_query = ", ".join(['"%s"' % k for k in item.keys()])
    #     values_query = ", ".join(["'%s'" % v for v in item.values()])

    #     print("Insert Query:", 'INSERT INTO "%s" (%s) VALUES (%s)' % (self.name, columns_query, values_query))
    #     print("Values:", item)
    #     # INSERT INTO "Agenda" ("Date", "Time_Start", ...) VALUES ('06/16/2018', '07:30 AM', ...)
    #     cursor = self.db_conn.cursor()
    #     cursor.execute('INSERT INTO "%s" (%s) VALUES (%s)' % (self.name, columns_query, values_query))
    #     cursor.close()
    #     self.db_conn.commit()
    ...
    def insert(self, item):
        # Replace space and special characters in column names
        item = {f'"{k}"'.replace('"', '""') if ' ' in k or '-' in k else k: v for k, v in item.items()}

        # build columns & values queries
        columns_query = ', '.join(['"{}"'.format(k.replace('"', '')) for k in item.keys()])
        values_query = ', '.join(["'%s'" % v if v is not None else 'NULL' for v in item.values()])

        # INSERT INTO "Agenda" ("Date", "Time Start", ...) VALUES ('None', 'None', ...)
        cursor = self.db_conn.cursor()
        insert_query = 'INSERT INTO "{}" ({}) VALUES ({})'.format(self.name, columns_query, values_query)
